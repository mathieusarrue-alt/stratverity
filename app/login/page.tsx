  ...
  if (provider === "Google") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <path fill="#ea4335" d="M12 6c1.5 0 2.9.5 4 1.6l3-3A10 10 0 0 0 3 7.4l3.4 2.7C7.2 7.7 9.4 6 12 6Z" />
      </svg>
    );
  }
  if (provider === "Apple") {
    ...
    <main className={styles.page}>
      <div className={styles.shell}>
        <section className={styles.intro}>
          <span className={styles.eyebrow}>ACCÈS STRATVERITY · SANS MOT DE PASSE SUPPLÉMENTAIRE</span>
          <h1>
            Un accès rapide.
            <em>Vos preuves au même endroit.</em>
          </h1>
          <p>
            Le même parcours sert à créer votre accès ou à vous reconnecter.
            Aucun nouveau mot de passe StratVerity à retenir.
          </p>
          <ul className={styles.trustList}>
            <li>Connexion sécurisée</li>
            <li>Identité minimale</li>
            <li>Aucun accès à vos conversations</li>
          </ul>
        </section>

        <section className={styles.card} aria-labelledby="login-title" data-premium-surface>
          <span className={styles.cardLabel}>{user ? "SESSION RECONNUE" : "CONNEXION OU INSCRIPTION"}</span>
          <h2 id="login-title">{user ? `Bienvenue, ${user.displayName}.` : "Continuer en quelques secondes."}</h2>
          <p>
            {user
              ? "Votre identité est déjà vérifiée. Vous pouvez ouvrir votre espace."
          ...
          {!user && (
            <>
              <p className={styles.providerCaption}>Choisir une méthode de connexion</p>
              <div className={styles.providers} aria-label="Méthodes de connexion disponibles">
                {["Google", "Apple", "Microsoft", "Email"].map((provider) => (
                  <Link
                    className={styles.providerLink}
                    href={destination}
                    key={provider}
                    aria-label={`Ouvrir l’écran sécurisé et continuer avec ${provider}`}
                  >
              ...
              <p className={styles.providerNote}>
                GitHub direct sera proposé uniquement après activation d’un fournisseur OAuth dédié.
              </p>
            </>
          )}
