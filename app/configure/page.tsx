...
export default function ScopeConfiguratorPage() {
  ...
  const chooseStrategies = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      promoteEssentialForScope(
        nextStrategies.length * assets.length * timeframes.length,
      );
      setState("idle");
      setNotice(null);
      setPreview(null);
  ...
  const toggleAsset = (asset: string) => {
    ...
    promoteEssentialForScope(
      projectedStrategyCount * nextAssetCount * timeframes.length,
    );
    setAssets((current) => {
  ...
  const toggleTimeframe = (timeframe: string) => {
    ...
    const nextTimeframeCount = timeframes.includes(timeframe)
      ...
      : timeframes.length + 1;
    promoteEssentialForScope(
      projectedStrategyCount * assets.length * nextTimeframeCount,
    );
    ...
    try {
      ...
      for (const strategy of strategies) {
        if (!strategy.file) {
          ...
          setFileError(
            locale === "fr"
              ? "Fichier source manquant en mémoire. Re-sélectionnez votre stratégie."
              : "Source file missing in memory. Re-select your strategy file.",
          );
          return;
        }
        ...
        if (!uploaded.ok) {
          ...
          if (uploaded.status === 404) {
            setFileError(
              locale === "fr"
                ? "Dépôt source indisponible (endpoint checkout-artifacts pas encore en prod). Paiement bloqué volontairement."
                : "Source deposit unavailable (checkout-artifacts not in prod yet). Payment blocked on purpose.",
            );
          } else {
            setFileError(
              locale === "fr"
                ? `Impossible de déposer le code source avant paiement (${uploaded.code || uploaded.status}).`
                : `Could not deposit strategy source before payment (${uploaded.code || uploaded.status}).`,
            );
          }
          return;
        }
        ...
        <div className={styles.builder}>
          ...
          <fieldset className={styles.block} data-premium-surface>
            ...
            <label className={styles.filePicker}>
              ...
              <small>
                {locale === "fr"
                  ? "Pine · Python · MQL4/5 · Notebook · ZIP · max 10 · dépôt source avant paiement"
                  : "Pine · Python · MQL4/5 · Notebook · ZIP · max 10 · source deposited before payment"}
              </small>
            </label>
            {strategies.length > 0 ? (
              <ul className={styles.strategyList}>
                {strategies.map((strategy, index) => (
                  <li key={strategy.sha256}>
                    ...
                    <button
                      aria-label={t("configure.removeStrategy", {
                        name: strategy.name,
                      })}
                      onClick={() => {
                        setStrategies((current) =>
                          current.filter(
                            (item) => item.sha256 !== strategy.sha256,
                          ),
                        );
                        setPreview(null);
                      }}
        ...
        <aside className={styles.summary} data-premium-surface>
          ...
          <div className={styles.livePrice}>
            ...
            <strong>
              ...
              <small>
                {price.cadence === "MONTHLY"
                  ? t("configure.perMonth")
                  : t("configure.totalSuffix")}
              </small>
            </strong>
            ...
            {price.cadence === "MONTHLY" ? (
              <div>
                <b>
                  {t("configure.firstPayment", {
                    amount: formatLocalizedPrice(price.dueTodayCents),
                  })}
                </b>
                <small>
              ...
              <div>
                {t("configure.oneTimePayment")} ·{" "}
                <b>{t("configure.noRenewal")}</b>
              </div>
            )}
          </div>
          ...
          <dl className={styles.metrics}>
            <div>
              ...
              <dd>
                {product === "AUDIT"
                  ? t("configure.auditOneTime")
                  : t("configure.scanRecurring")}
              </dd>
            </div>
            <div>
          ...
          <div
            className={styles.limitTrack}
            aria-label={t("configure.limitUsage")}
          >
            <i
              style={{
                width: `${Math.min(
          ...
          <p
            ...
            className={`${styles.message} ${
              state === "error" || state === "fallback" || fileError
                ? styles.warning
                : ""
            }`}
          >
          ...
          {preview ? (
            <div className={styles.serverProof}>
              ...
              <dl>
                <div>
                  <dt>{t("configure.contexts")}</dt>
                  <dd>{preview.context_count.toLocaleString(locale)}</dd>
                </div>
                <div>
              ...
              <label className={styles.contractAcceptance}>
                ...
                <span>
                  ...
                  {t("configure.contract.beforePrivacy")}{" "}
                  <Link href="/legal/privacy">{t("common.privacy")}</Link>{" "}
                  {t("configure.contract.beforeLicense")}{" "}
                  <Link href="/legal/content-license">
                    {t("common.contentLicense")}
                  </Link>
              ...
              <button
                ...
                disabled={
                  state === "checkout" || product === "SCAN" || !contractAccepted
                }
